...
type SDDP_FileHandle struct {
	...
	// parallel read
	buffers           []*SDDP_S3ReadBuffer
	existingReadahead int
	seqReadAmount     uint64
	numOOORead        uint64 // number of out of order read
}

const SDDP_MAX_READAHEAD = uint32(100 * 1024 * 1024)
const SDDP_READAHEAD_CHUNK = uint32(20 * 1024 * 1024)

func SDDP_NewFileHandle(in *SDDP_Inode) *SDDP_FileHandle {
	fh := &SDDP_FileHandle{inode: in}
	return fh
}

func (fh *SDDP_FileHandle) initWrite() {
...
func (fh *SDDP_FileHandle) initMPU() {
	defer func() {
		fh.mpuWG.Done()
	}()
...
func (fh *SDDP_FileHandle) mpuPartNoSpawn(buf *MBuf, part int) (err error) {
...
func (fh *SDDP_FileHandle) mpuPart(buf *MBuf, part int) {
	defer func() {
		fh.mpuWG.Done()
	}()
...
func (fh *SDDP_FileHandle) waitForCreateMPU() (err error) {
	if fh.mpuId == nil {
		fh.mu.Unlock()
		fh.initWrite()
...
func (fh *SDDP_FileHandle) partSize() uint64 {
...
func (fh *SDDP_FileHandle) WriteFile(offset int64, data []byte) (err error) {
...
func (b SDDP_S3ReadBuffer) Init(fh *SDDP_FileHandle, offset uint64, size uint32) *SDDP_S3ReadBuffer {
	fs := fh.inode.fs
	b.s3 = fs.s3
	b.offset = offset
	b.size = size
...
func (b *SDDP_S3ReadBuffer) Read(offset uint64, p []byte) (n int, err error) {
	if b.offset == offset {
...
func (fh *SDDP_FileHandle) readFromReadAhead(offset uint64, buf []byte) (bytesRead int, err error) {
	var nread int
...
func (fh *SDDP_FileHandle) readAhead(offset uint64, needAtLeast int) (err error) {
	existingReadahead := uint32(0)
	for _, b := range fh.buffers {
		existingReadahead += b.size
	}

	readAheadAmount := SDDP_MAX_READAHEAD

	for readAheadAmount-existingReadahead >= SDDP_READAHEAD_CHUNK {
		off := offset + uint64(existingReadahead)
		remaining := fh.inode.Attributes.Size - off

		// only read up to readahead chunk each time
		size := MinUInt32(readAheadAmount-existingReadahead, SDDP_READAHEAD_CHUNK)
		...
		if size != 0 {
			...
			readAheadBuf := SDDP_S3ReadBuffer{}.Init(fh, off, size)
			if readAheadBuf != nil {
...
func (fh *SDDP_FileHandle) readFile(offset int64, buf []byte) (bytesRead int, err error) {
	...
	if !fs.flags.Cheap && fh.seqReadAmount >= uint64(SDDP_READAHEAD_CHUNK) && fh.numOOORead < 3 {
		if fh.reader != nil {
...
func (fh *SDDP_FileHandle) readFromStream(offset int64, buf []byte) (bytesRead int, err error) {
	defer func() {
...
func (fh *SDDP_FileHandle) resetToKnownSize() {
...
func (fh *SDDP_FileHandle) FlushFile() (err error) {
	fh.mu.Lock()
	defer fh.mu.Unlock()
